...
export default function ImporterCertificationPanel({ companyId, year, officialFiles }) {
  ...
  const validatedCore = CORE_CIRCUITS.filter(code => circuitStatus(code)[0] === 'Muestra aceptada y revisada').length;
  const validatedExtended = EXTENDED_CIRCUITS.filter(code => circuitStatus(code)[0] === 'Muestra aceptada y revisada').length;
  const rejectedCircuits = [...CORE_CIRCUITS, ...EXTENDED_CIRCUITS].filter(code => circuitStatus(code)[0].startsWith('Importador rechazó')).length;
  return <section className="rounded-2xl border border-cyan-200 bg-white p-5">
    ...
    <div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-cyan-200 bg-cyan-50 p-3 text-xs text-cyan-900">Muestras aceptadas y revisadas: <strong>{validatedCore}/15</strong> circuitos principales</div><div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-800">Ampliación: <strong>{validatedExtended}/3</strong> modelos</div><div className={`rounded-xl border p-3 text-xs ${rejectedCircuits ? 'border-red-200 bg-red-50 text-red-800' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>Circuitos con rechazo: <strong>{rejectedCircuits}</strong></div></div>
    <p className="mt-2 text-[11px] leading-5 text-slate-500">Estos contadores certifican muestras y diseños concretos; no sustituyen las pruebas por ejercicio, régimen, rol ni los anexos A/B del 296.</p>
    <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-semibold text-slate-800">15 circuitos originales: 14 AEAT + 415 ATC</p><p className="mt-1 text-[11px] text-slate-500">El estado se refiere solo a muestras del ejercicio seleccionado, no a la compatibilidad universal de otras campañas o períodos.</p><div className="mt-3 grid gap-2 sm:grid-cols-3 lg:grid-cols-5">{CORE_CIRCUITS.map(code => { const [label, tone] = circuitStatus(code); return <div key={code} className="rounded-lg border border-slate-200 bg-white p-2"><p className="text-xs font-semibold">Modelo {code}{code === '415' ? ' · ATC' : ' · AEAT'}</p><p className={`mt-1 text-[11px] ${tone}`}>{label}</p></div>; })}</div><p className="mt-3 text-xs font-semibold text-slate-800">Ampliación estructurada: 200, 202 y 232</p><div className="mt-2 grid gap-2 sm:grid-cols-3">{EXTENDED_CIRCUITS.map(code => { const [label, tone] = circuitStatus(code); return <div key={code} className="rounded-lg border border-slate-200 bg-white p-2"><p className="text-xs font-semibold">Modelo {code} · AEAT</p><p className={`mt-1 text-[11px] ${tone}`}>{label}</p></div>; })}</div></div>
