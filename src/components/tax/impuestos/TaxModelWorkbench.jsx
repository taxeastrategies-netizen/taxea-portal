...
const ADJUSTMENT_MODELS = ['130', '131', '200', '202', '216', '232', '303', '417', '420', '421'];
...
const STRUCTURED_ADJUSTMENTS = {
  ...
  '200': {
    title: 'Cierre y diseño oficial del modelo 200',
    help: 'Diseño AEAT DR200e25 v1.02. Taxea precarga la conciliación y las casillas principales; completa balances, estado de patrimonio, ajustes y anexos aplicables con el mapa página.casilla antes de importar el fichero en Sociedades WEB.',
    fields: [
      ['fiscalPeriodStart','Inicio del período impositivo','date'],['fiscalPeriodEnd','Fin del período impositivo','date'],['exerciseType','Tipo de ejercicio','select',[['1','1 · 12 meses año natural'],['2','2 · 12 meses, 365 días'],['3','3 · Inferior a 12 meses']]],['cnae','CNAE-2025 principal (4 dígitos)','text'],
      ['statementDiscriminator','Tipo de entidad / discriminante','select',[['0','0 · Normal, abreviado o PYME'],['A','A · Aseguradora'],['E','E · Entidad de crédito'],['I','I · Inversión colectiva'],['G','G · Garantía recíproca']]],['declarationType','Resultado de declaración','select',[['','Automático'],['I','I · A ingresar'],['D','D · A devolver'],['N','N · Cero / negativa']]],['contactPhone1','Teléfono de contacto','text'],['contactPhone2','Segundo teléfono','text'],
      ['taxableIncreases','Ajustes extracontables positivos','number'],['taxableDecreases','Ajustes extracontables negativos','number'],['taxLossCarryforward','BIN a compensar','number'],['taxRate','Tipo de gravamen (%)','number'],['taxCredits','Bonificaciones y deducciones','number'],['withholdings','Retenciones e ingresos a cuenta','number'],['instalmentPayment1','Primer pago fraccionado 202','number'],['instalmentPayment2','Segundo pago fraccionado 202','number'],['instalmentPayment3','Tercer pago fraccionado 202','number'],['instalmentPayments','Total 202 si no se desglosa','number'],['previousSamePeriodResult','Resultado previo rectificativa','number'],
      ['rectifyingDeclaration','Autoliquidación rectificativa','checkbox'],['administrativeDiscrepancy','Rectificación por discrepancia administrativa','checkbox'],['previousReceiptNumber','Justificante anterior','text'],['iban','IBAN para ingreso/devolución','text'],['officialDataReviewed','Mapa oficial revisado por asesor','checkbox'],
      ['model200PageCodesText','Páginas aplicables (códigos separados por coma)','textarea'],['model200OfficialBoxesText','Casillas adicionales · una por línea: PAGINA.CASILLA=IMPORTE','textarea'],['model200OfficialTextFieldsText','Campos alfanuméricos expertos · PAGINA.POSICION.LONGITUD=VALOR','textarea'],
    ],
  },
  '202': {
    title: 'Modalidad y magnitudes del modelo 202',
    help: 'Diseño oficial AEAT DR202e25 v1.3. Confirma la modalidad censal, el CNAE-2025 y los datos societarios. Las casillas calculadas se muestran después con trazabilidad; los datos especiales solo se informan cuando procedan.',
    fields: [
      ['method','Modalidad','select',[['','Seleccionar'],['40_2','Artículo 40.2 LIS'],['40_3','Artículo 40.3 LIS']]],['fiscalPeriodStart','Inicio período impositivo','date'],['cnae','CNAE-2025 principal (4 dígitos)','text'],['foralRegimeCode','Normativa foral','select',[['','No aplica'],['1','Navarra'],['2','Guipúzcoa'],['3','Vizcaya'],['4','Álava']]],
      ['corporateTaxRateText','Tipo/s IS del ejercicio','text'],['turnoverBand','Cifra de negocios previa','select',[['0','No consta / menos de 10 M€'],['1','10 M€ a menos de 20 M€'],['2','20 M€ a menos de 60 M€'],['3','60 M€ o más']]],['commonTerritoryPercentage','Operaciones en territorio común (%) · 29','number'],
      ['nonProfitRegime','Régimen Ley 49/2002','checkbox'],['socimiRegime','Régimen SOCIMI','checkbox'],['ventureCapitalRegime','Entidad de capital-riesgo · art. 50 LIS','checkbox'],['tonnageRegime','Régimen de tonelaje','checkbox'],['smallCompanyIncentives','Incentivos empresa reducida dimensión','checkbox'],['turnoverOverSixMillion','Cifra de negocios superior a 6 M€','checkbox'],['protectedCooperative','Cooperativa fiscalmente protegida','checkbox'],['specialCircumstances','RIC/ZEC/bonificación u otra circunstancia especial','checkbox'],['turnoverUnderOneMillion','Cifra de negocios inferior a 1 M€','checkbox'],['multipleTaxRates','Más de un tipo impositivo','checkbox'],
      ['previousCorporateTaxQuota','Base del pago fraccionado 40.2 · 01','number'],['previousSamePeriodResult','Ingresado antes en complementaria · 02/31','number'],
      ['accountingResult','Resultado contable después de IS e IC · 04','number'],['currentTaxableBase','Base acumulada directa si no se detallan ajustes','number'],['corporateTaxIncrease','Corrección IS · aumentos · 05','number'],['corporateTaxDecrease','Corrección IS · disminuciones · 06','number'],['complementaryTaxIncrease','Impuesto complementario · aumentos · 67','number'],['amortizationReversalDecrease','Reversión amortización · disminuciones · 37','number'],['otherTaxIncreases','Resto correcciones · aumentos · 07','number'],['otherTaxDecreases','Resto correcciones · disminuciones · 08','number'],['capitalizationReserveCarryforward','Remanente reserva capitalización · 44','number'],['taxLossCarryforward','BIN a compensar · 14','number'],['equalizationReserveIncrease','Reserva nivelación · aumentos · 45','number'],['equalizationReserveDecrease','Reserva nivelación · disminuciones · 46','number'],
      ['paymentPercentage','Porcentaje aplicable · 17','number'],['cooperativeArticle1112Quota','Dotaciones art. 11.12 cooperativas · 47','number'],['cooperativeNegativeQuota','Cuotas negativas cooperativas · 40','number'],['equalizationReserveQuotaIncrease','Reserva nivelación en cuota · aumenta · 48','number'],['equalizationReserveQuotaDecrease','Reserva nivelación en cuota · disminuye · 49','number'],
      ['multipleRateBase1','Base tipo 1 · 20','number'],['multipleRatePercentage1','Porcentaje tipo 1 · 21','number'],['multipleRateBase2','Base tipo 2 · 23','number'],['multipleRatePercentage2','Porcentaje tipo 2 · 24','number'],['multipleRateBase3','Base tipo 3 · 61','number'],['multipleRatePercentage3','Porcentaje tipo 3 · 62','number'],['multipleRateBase4','Base tipo 4 · 64','number'],['multipleRatePercentage4','Porcentaje tipo 4 · 65','number'],['multipleRateCooperativeArticle1112Quota','Dotaciones cooperativas varios tipos · 50','number'],['multipleRateCooperativeNegativeQuota','Cuotas negativas varios tipos · 42','number'],['multipleRateEqualizationQuotaIncrease','Reserva nivelación cuota aumenta · 51','number'],['multipleRateEqualizationQuotaDecrease','Reserva nivelación cuota disminuye · 52','number'],
      ['bonuses','Bonificaciones · 27','number'],['withholdings','Retenciones · 28','number'],['previousInstalmentPayments','Pagos anteriores del ejercicio · 30','number'],['minimumPayment','Pago mínimo aplicable · 33','number'],
      ['additionalInfo','Comunicación de datos adicionales','checkbox'],['sociedadesReference','NRS de la comunicación','text'],['excludedDebtRestructuringIncome','Quita/espera excluida del mínimo','number'],['integratedDebtRestructuringIncome','Quita/espera integrada en base','number'],['integratedDebtRestructuringQuota','Quita/espera integrada en cuota','number'],['reversalImpairmentIncome','Reversión de deterioros','number'],['canaryInvestmentReserveAmount','Reserva para Inversiones en Canarias','number'],['canaryArticle26BonusAmount','Bonificación art. 26 Ley 19/1994','number'],['zecExcludedAmount','Importe no computable ZEC','number'],['ceutaMelillaReductionAmount','Minoración bonificación Ceuta/Melilla','number'],['capitalIncreaseCreditCompensationExcludedAmount','Aumento de capital excluido · art. 17.2 LIS','number'],['partiallyExemptIncomeAmount','Renta exenta entidades parcialmente exentas','number'],['article34BonusAmount','Bonificación art. 34 LIS','number'],
      ['complementaryDeclaration','Declaración complementaria','checkbox'],['previousReceiptNumber','Justificante anterior','text'],['declarationPaymentType','Forma de ingreso','select',[['','Automática'],['I','Ingreso'],['U','Domiciliación'],['G','Ingreso en CCT'],['N','Negativa / cero']]],['iban','IBAN español para domiciliación','text'],
    ],
  },
  '216': {
  ...
  '232': {
    title: 'Devengo y cabecera oficial del modelo 232',
    help: 'Diseño AEAT v1.4. Confirma ejercicio, CNAE, período y, si procede, la declaración complementaria o sustitutiva y la matriz. Las operaciones se detallan individualmente en el bloque inferior.',
    fields: [['fiscalPeriodStart','Inicio del período impositivo','date'],['fiscalPeriodEnd','Fin del período impositivo','date'],['exerciseType','Tipo de ejercicio','select',[['1','1 · 12 meses año natural'],['2','2 · 12 meses, 365 días'],['3','3 · Inferior a 12 meses']]],['cnae','CNAE actividad principal','text'],['declarationCorrectionType','Corrección','select',[['','No aplica'],['C','C · Complementaria'],['S','S · Sustitutiva']]],['previousReceiptNumber','Justificante anterior','text'],['parentTaxId','Identificación de la matriz','text'],['parentName','Razón social de la matriz','text']],
  },
  '421': {
...
const DECLARABLE_SCHEMAS = {
  ...
  '232': [
    ['category','Bloque','select',[['related','Operación vinculada'],['patent_box','Patent box'],['tax_haven','Operación con territorio no cooperativo'],['tax_haven_holding','Tenencia de valores en territorio no cooperativo']]],['relatedPartyTaxId','NIF / identificación de la contraparte','text'],['relatedPartyName','Nombre o razón social','text'],['entityTypeKey','Tipo de persona','select',[['','Seleccionar'],['F','F · Persona física'],['J','J · Persona jurídica'],['O','O · Otra']]],
    ['countryOrProvinceCode','Código provincia o país (2 caracteres)','text'],['relationType','Tipo de vinculación','select',[['','Seleccionar'],['A','A · Entidad y socios'],['B','B · Entidad y administradores'],['C','C · Familiares vinculados'],['D','D · Entidades del grupo'],['E','E · Administradores de entidad del grupo'],['F','F · Participación indirecta ≥25%'],['G','G · Socios comunes ≥25%'],['H','H · Entidad y establecimiento permanente']]],['operationType','Tipo de operación','select',[['','Seleccionar'],['01','01 · Bienes tangibles'],['02','02 · Intangibles'],['03','03 · Fondos propios'],['04','04 · Créditos y deuda'],['05','05 · Operaciones financieras'],['06','06 · Servicios'],['07','07 · Reparto de costes'],['08','08 · Inmuebles'],['09','09 · Intereses'],['10','10 · Trabajo y pensiones'],['11','11 · Otras']]],['valuationMethod','Método de valoración','select',[['','Seleccionar'],['1A','1A · Precio libre comparable'],['1B','1B · Coste incrementado'],['1C','1C · Precio de reventa'],['1D','1D · Distribución del resultado'],['1E','1E · Margen neto operacional']]],
    ['incomePayment','Ingreso o pago','select',[['','Seleccionar'],['I','I · Ingreso'],['P','P · Pago']]],['operationDate','Fecha de operación','date'],['amount','Importe de operación','number'],['operationDescription','Descripción de operación en territorio no cooperativo','text'],['holdingType','Tipo de tenencia','select',[['','Seleccionar'],['A','A · Fondos propios'],['B','B · Institución de inversión colectiva'],['C','C · Renta fija cotizada']]],['acquisitionValue','Valor de adquisición','number'],['participationRate','Participación (%)','number'],['specialDataConfirmed','Registro oficial revisado','checkbox'],
  ],
};
...
const MODEL_296_ANNEX_A_FIELDS = [
  ...
  ['netPayment','Pago neto tras retención','number'],['withholdingRate','Tipo de retención (%)','number'],['withholdingAmount','Retención reembolsada','number'],
  ['address','Domicilio','text'],['addressComplement','Complemento de domicilio','text'],['city','Ciudad','text'],['region','Provincia o región','text'],['postalCode','Código postal','text'],['addressCountry','País del domicilio ISO','text'],
  ['model210Receipt','Justificante del modelo 210 (13 dígitos)','text'],['foreignTaxId','Identificador fiscal extranjero','text'],
...
const MODEL_296_ANNEX_B_FIELDS = [
  ...
  ['grossIncome','Rendimiento íntegro','number'],['withholdingAmount','Retención','number'],['withholdingRate','Tipo de retención (%)','number'],['model210PresentationDate','Fecha de presentación del 210','date'],
];
  ...
  return (
    <section className="rounded-2xl border border-blue-200 bg-blue-50/50 p-4">
      ...
      {open && <div className="mt-4 space-y-3 rounded-xl border border-blue-200 bg-white p-4">
        <label className="block cursor-pointer rounded-xl border border-dashed border-blue-300 bg-blue-50/40 p-4 text-center text-xs text-blue-800"><input type="file" className="hidden" accept=".pdf,.txt,.111,.115,.123,.130,.131,.180,.190,.193,.200,.202,.216,.232,.296,.303,.347,.349,.390,.415,.417,.420,.421,.425,application/pdf,text/plain" onChange={event => analyzeFile(event.target.files?.[0])} />{working ? <span className="inline-flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Analizando…</span> : fileInfo ? fileInfo.fileName : 'Seleccionar fichero oficial o justificante PDF'}</label>
        {fileInfo && <>
          ...
          {fileInfo.preview?.source === 'fichero_no_reconocido' && <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">El fichero no sigue un diseño que Taxea reconozca. Puedes registrar el histórico contrastando el justificante y las casillas, pero quedará pendiente de revisión, no como importación estructural validada.</p>}
          {['180','190','193','232','296','347','349','415'].includes(modelCode) && form.declarationType === 'complementaria' && <p className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-xs text-blue-800">Complementaria informativa: introduce solo los perceptores u operaciones adicionales y sus importes. Taxea los añadirá a las versiones presentadas; no vuelvas a introducir el total completo del original.</p>}
          {['180','190','193','232','296','347','349','415'].includes(modelCode) && form.declarationType === 'sustitutiva' && <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">Sustitutiva informativa: incluye el conjunto íntegro que reemplaza al anterior. Verifica que la declaración sustituida se haya dado de baja cuando el procedimiento lo exija y conserva ambos justificantes.</p>}
    ...
    <section className="space-y-4 rounded-2xl border border-indigo-200 bg-indigo-50/40 p-4">
      ...
      {history?.presented && <div className="rounded-xl border border-amber-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div><p className="text-xs font-semibold uppercase tracking-wide text-amber-700">Período ya presentado</p><p className="mt-1 text-sm text-slate-700">{history.filing?.date || 'Fecha no disponible'} · versión {history.filing?.snapshotVersion || 1}{history.filing?.justificationNumber ? ` · justificante ${history.filing.justificationNumber}` : ''}</p>{history.filing?.activeVersionIds?.length > 1 && <p className="mt-1 text-xs text-blue-700">Importes efectivos de {history.filing.activeVersionIds.length} versiones activas; el original y las correcciones permanecen guardados por separado.</p>}{history.filing?.correctionIncomplete && <p className="mt-1 text-xs text-red-700">Falta la declaración original para reconstruir el conjunto presentado. Revisa este histórico antes de utilizar sus importes.</p>}</div>
...
function StructuredAdjustmentPanel({ modelCode, values, onChange }) {
  ...
  return <section className="rounded-2xl border border-cyan-200 bg-cyan-50/60 p-4">
    <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-cyan-700" /><div className="min-w-0 flex-1">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{config.fields.map(([key, label, type, options]) => <label key={key} className={type === 'checkbox' ? 'flex items-center gap-2 rounded-lg border border-cyan-200 bg-white px-3 py-2 text-xs text-slate-700' : `${type === 'textarea' ? 'md:col-span-2 xl:col-span-4 ' : ''}text-xs font-medium text-slate-600`}>
        {type === 'checkbox' ? <><input type="checkbox" checked={values[key] === true} onChange={event => onChange(key, event.target.checked)} />{label}</> : <>{label}{type === 'select' ? <select value={values[key] ?? ''} onChange={event => onChange(key, event.target.value || undefined)} className="mt-1 h-9 w-full rounded-lg border border-cyan-200 bg-white px-3 text-sm text-slate-800">{options.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select> : type === 'textarea' ? <textarea rows={4} value={values[key] ?? ''} onChange={event => onChange(key, event.target.value || undefined)} className="mt-1 w-full rounded-lg border border-cyan-200 bg-white px-3 py-2 font-mono text-xs text-slate-800 outline-none focus:ring-2 focus:ring-cyan-300" /> : <input type={type === 'number' ? 'number' : type} step={type === 'number' ? '0.01' : undefined} value={values[key] ?? ''} onChange={event => onChange(key, event.target.value === '' ? undefined : type === 'number' ? Number(event.target.value) : event.target.value)} className="mt-1 h-9 w-full rounded-lg border border-cyan-200 bg-white px-3 text-sm text-slate-800 outline-none focus:ring-2 focus:ring-cyan-300" />}</>}
      </label>)}</div>
    </div></div>
  </section>;
}
    ...
    <div className="grid min-h-[690px] grid-cols-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 bg-gradient-to-br from-slate-50 via-white to-cyan-50/40">
        ...
        <div className="space-y-5 p-5">
          ...
          {catalogResponse?.readiness?.developerTaxIdValid === false && ['111','115','123','130','131','202','216','303','390'].includes(modelCode) && (
            <div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><div><p className="font-semibold">Identificación del desarrollador pendiente</p><p className="mt-1 text-xs leading-5">{catalogResponse?.readiness?.developerTaxIdConfigured ? 'Corrige' : 'Configura'} el secreto <code>TAXEA_DEVELOPER_NIF</code> en Base44 con un NIF español válido. Su valor nunca se muestra en el portal; solo completa el bloque auxiliar del fichero AEAT.</p></div></div>
          )}
          {lastExportInfo && (
