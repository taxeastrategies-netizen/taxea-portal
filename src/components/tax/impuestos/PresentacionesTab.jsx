...
function EvidenceEditor({ filing, companyId, onSaved, onCancel }) {
  ...
  const save = useMutation({
    ...
    onSuccess: onSaved,
    onError: err => setError(err?.response?.data?.error || err?.message || 'No se pudo guardar la evidencia.'),
  });
...
export default function PresentacionesTab({ onOpenModel }) {
  ...
  const filings = useMemo(() => [...(allVersions ? workspace.data?.filings || [] : workspace.data?.latestFilings || [])].sort((a, b) => new Date(b.fechaPresentacion || b.fechaImportacion || 0) - new Date(a.fechaPresentacion || a.fechaImportacion || 0)), [workspace.data, allVersions]);
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="text-sm font-semibold text-slate-900">Ficheros generados</h3><p className="mt-1 text-xs text-slate-500">Trazabilidad de exportaciones. Su estado nunca convierte por sí solo el periodo en presentado.</p></div><span className="text-xs text-slate-400">{officialFiles.length} registros</span></div>{officialFiles.length === 0 ? <p className="mt-4 rounded-lg bg-slate-50 p-4 text-xs text-slate-500">No se generaron ficheros en {year}.</p> : <div className="mt-4 divide-y divide-slate-100">{officialFiles.map(file => <div key={file.id} className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium text-slate-800">{file.nombreFichero || `Modelo ${file.modeloCodigo}`}</p><p className="text-xs text-slate-500">Modelo {file.modeloCodigo} · {file.periodo} {file.ejercicio} · {file.formato} · {file.estado}</p></div><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-[10px] text-slate-400">{file.hash?.slice(0, 16)}…</span>{file.fileUrl && <a href={file.fileUrl} target="_blank" rel="noreferrer" className="text-xs font-medium text-cyan-700">Abrir archivo</a>}<Button size="sm" variant="ghost" onClick={() => onOpenModel?.({ modelCode: file.modeloCodigo, year: file.ejercicio, period: file.periodo })}>Abrir modelo <ArrowRight className="ml-1 h-3.5 w-3.5" /></Button></div></div>)}</div>}</section>
  </div>;
}
