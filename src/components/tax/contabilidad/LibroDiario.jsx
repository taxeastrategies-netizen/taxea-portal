...
export default function LibroDiario({ companyId, user, initialSource = 'all' }) {
  ...
  return <div className="space-y-4">
    ...
    {quality && <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">{[['Asientos del filtro', journal?.total ?? quality.entries], ['Confirmados', quality.confirmedEntries], ['En revisión', quality.reviewEntries], ['Excluidos por integridad', quality.entriesWithoutLines + quality.unbalancedEntries]].map(([label,value]) => <div key={label} className="rounded-lg border border-border bg-card px-3 py-2"><p className="text-[10px] text-muted-foreground">{label}</p><p className="font-mono text-sm font-bold">{value}</p></div>)}</div>}
