...
import { BarChart2, Upload, FileText, AlertTriangle, CheckCircle2, Clock, ChevronRight, Info } from 'lucide-react';
...
const CALIDAD = { alta: { label: 'Alta', color: 'text-emerald-600' }, media: { label: 'Media', color: 'text-amber-600' }, baja: { label: 'Baja', color: 'text-red-600' }, sin_evaluar: { label: '—', color: 'text-slate-400' } };
const ORIGEN_LABEL = { a3: 'A3 .dat', excel_balance: 'Excel Balance', excel_pyg: 'Excel PyG', excel_diario: 'Excel Diario', excel_combined: 'Excel combinado', csv_generico: 'CSV' };
...
export default function AnalysisHome() {
  ...
  const [view, setView] = useState('home'); // home | selector | import_dat | import_excel | dashboard | report
  ...
  const handleSelectType = (type) => {
    setImportType(type);
    setView(type === 'dat' ? 'import_dat' : 'import_excel');
  };

  if (view === 'selector') return <ImportSelector onSelect={handleSelectType} onCancel={() => setView('home')} />;
  ...
  if (view === 'report' && selectedReport) return <ReportViewer report={selectedReport} imp={selectedImport} onBack={() => setView('dashboard')} />;
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-10">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Importa contabilidad desde A3 .dat o Excel (Balance, PyG, Diario) y genera informes financieros detallados.</p>
        </div>
      ...
      <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-5 py-4">
        ...
        <p className="text-xs text-blue-700 leading-relaxed">
          <strong>Análisis preliminar basado en los datos importados.</strong> Los resultados dependen de la calidad y completitud del archivo. No constituye auditoría, certificación contable ni validación fiscal definitiva. Toda clasificación es estimada. Consulta con tu asesor profesional.
        </p>
      </div>
      ...
      {/* Import list */}
      {loading ? (
        <div className="flex items-center justify-center h-32"><div className="w-6 h-6 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" /></div>
      ) : imports.length === 0 ? (
        ...
        <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-50">
            {imports.map((imp, i) => {
              ...
              return (
                <div key={imp.id || i} className="px-5 py-4 flex items-center gap-4 hover:bg-slate-50/50 cursor-pointer transition-colors"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400">{ORIGEN_LABEL[imp.origen] || imp.origen} · {imp.empresa_nombre || 'Sin empresa'} · {imp.total_lineas || 0} líneas</p>
                  </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { icon: Upload, title: 'A3 .dat — Parser robusto', desc: 'Multi-estrategia con detección de codificación, estructura y diagnóstico de líneas no reconocidas.', color: 'text-emerald-600', bg: 'bg-emerald-50', action: 'dat' },
          { icon: BarChart2, title: 'Excel Balance, PyG, Diario', desc: 'Importa uno o los tres. El análisis se adapta y cruza los datos para validar coherencia.', color: 'text-blue-600', bg: 'bg-blue-50', action: 'excel_combined' },
          { icon: FileText, title: 'Informe financiero detallado', desc: 'PyG, balance, caja, ratios, alertas y recomendaciones con trazabilidad del origen de cada dato.', color: 'text-violet-600', bg: 'bg-violet-50', action: null },
        ].map((f, i) => {
          const Icon = f.icon;
          return (
            <div key={i} className={cn("bg-white border border-slate-100 rounded-2xl p-5 shadow-sm", f.action && "cursor-pointer hover:shadow-md transition-all")}
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-3xl p-12 text-center shadow-sm">
      ...
      <p className="text-sm text-slate-500 mb-6 max-w-sm mx-auto">
        Sube un archivo .dat de A3 o un Excel de Balance, PyG o Diario. Revisamos la estructura antes de analizar.
      </p>
